...
var commApi = {
    ...
    "saveSubWay": '/Home/SaveDecisionPoint', // 保存地铁图关联
    ...
    "tpl": ['<tr id="tr{taskId}" cp="1" data-lvl="{treelvl}">',
		'<td field="torder" class="datagrid-td-rownumber"><div class="datagrid-cell-rownumber">{rowindex}</div></td>',
		'<td field="name" class="{cls}" width="300"><div style="padding-left:{padleft}px;padding-right:5px;float:left"><img style="{icon}" id="img{taskId}" src="' + __globalDomain + '/Content/images/DesignManage/ico14.jpg"></div><div style="float:left;width:{nwidth}px">{name}</div></td>',
		...
		'<td field="status" style="text-align:center" statId={statId} class="norbor">{statusName}</td>',
		'</tr>'],
...
function a2DataGrid(config) {
    ...
    this.bd = this.grid.find("table.gridCt");
    ...
    this.filterDs = { owners: {}, departs: {}, stat: { '2': '未开始', '3': '进行中', '4': '已完成'} };
    ...
    this.init = function () {
        ...
        $('#selTaskLevel').change(function () {
            var lvl = this.options[this.selectedIndex].value;
            if (lvl != -1) {
                setTimeout(function () {
                    var _rows = self.dt[0].rows, _tr, i = 0, _lvl, _img, _vsb;
                    for (; _tr = _rows[i]; ++i) {
                        _lvl = _tr.getAttribute('data-lvl');
                        _tr = $(_tr);
                        if (_lvl <= lvl) {
                            _img = _tr.find("img:first");
                            _vsb = _img[0].style.visibility;
                            if (!_vsb || _vsb != 'hidden') {
                                _img.attr("src", epcl[lvl == _lvl ? 1 : 0]);
                                _tr.attr("cp", lvl == _lvl ? "0" : "1");
                            }
                            _tr.show();
                        } else {
                            _tr.hide();
...
a2DataGrid.prototype.loadData = function () { // 加载数据
    ...
    gLoadQueue.add({
        ...
        success: function (rs) {
            ...
            self.kwFilter();
            self._renderFilter();
            self.grid.find("a.colall").trigger("click");
            self.grid.unmask();
        }
    });
};
a2DataGrid.prototype.kwFilter = function () {
    ...
    rc.find("th[field=levelId]").unbind('click').bind("click", function () {
        _gFilter(self, this);
    });
    rc.find("th[field=status]").unbind('click').bind("click", function () {
        _gFilter(self, this);
    });
};
a2DataGrid.prototype._renderFilter = function () {
    var self = this, tb = $('#gfiltertb');
    if (tb[0]) {
        var departs = self.filterDs.departs, key, str = '<tr><td style="background-color:#F7F5F5;" align="center">部门</td>', owners, i, _u, str1 = '<tr><td  style="background-color:#F7F5F5;" align="center">岗位</td>';

        for (key in departs) {
            owners = departs[key] || [];
            owners = owners.unique();
            str += '<td align="center" height="21"><a href="javascript:void(0);" name="g_dept" data="' + key + '">' + key + '</a></td>';
            str1 += '<td align="center" height="21">'
            for (i = 0; _u = owners[i]; ++i) {
                str1 += '<p><a href="javascript:void(0);" name="deptowner" data="' + key + '|DDD|' + _u + '">' + _u + '</a></p>';
            }
            str1 += '</td>';
        }
        str += '</tr>';
        ...
        if (str != '') {
            $('#gfiltertb_ct').html(str);
            tb.show();
        } else {
            tb.hide();
        }
        $('#gfiltertb_btn').off().on('click', function () {
            var rs = '';
            rs = "|YYY|";
            tb.find("a.this").each(function () {
                if (this.name == 'deptowner') rs += $(this).attr('data') + "|YYY|";
            });
            _doGFilterDept(self, rs);
            self.focusDiv.hide();
        });
        tb.off().on('click', 'a', function () {
            $(this).toggleClass('this');
            if (this.name == 'g_dept') {
                var p = $(this).parent(), idx = p.index();
                if ($(this).hasClass('this')) {
                    p.parent().next().find('td:eq(' + idx + ')').find("a").addClass('this');
                } else {
                    p.parent().next().find('td:eq(' + idx + ')').find("a").removeClass('this');
                }
            }
        });
    }
...
a2DataGrid.prototype.calcrs_ = function (dt, lvl) {
    ...
    if (dt['name'] != "请填写任务名称") {
        ...
        if ("章志刚 邓力 孙经理".indexOf(dt['ownerName']) != -1) {
            _cls = 'red';
        }
    ...
    dt["cls"] = _cls;
    dt["pointName"] = String.cut(dt["pointName"] || "", 10, '...');
...
a2DataGrid.prototype.renderRows = function (data, texp, pid, lvl, ino) { // 渲染
    ...
    _rowindex = 0;
    // filter
    var filterDs = self.filterDs, owners = filterDs.owners, departs = filterDs.departs;

    function rd(pid, lv) {
        var dt, i = 0, _tid, _pid, tmp, _owner, _orgN;
        for (; dt = ctsks[i]; ++i) {
            ...
            if (_pid == pid) {
                ...
                _orgN = dt['orgName'];
                if (_orgN != '') {
                    if (!departs[_orgN]) departs[_orgN] = [];
                }
                _owner = dt['ownerName'];
                if (_owner != '' && _orgN != '') {
                    departs[_orgN].push(_owner);
                }
                dt["rowindex"] = _rowindex;
                dt["treelvl"] = String(lv);
...
a2DataGrid.prototype.menu = function (e) { // menu
    ...
    _menu.find(".menu-link").unbind("click").bind("click", function () {

        box("/DesignManage/TaskRelationEdit?preTaskId=" + cell.tid + "&r=" + Math.random(), { boxid: "TaskRelationEdit", title: '编辑任务关联', contentType: 'ajax', width: 600,
            submit_cb: function () {
                saveLinkTask();
            }
        });
...
a2DataGrid.prototype.saveData = function (data) { // 保存 任务数据
    ...
    if (data.modified) {
        ...
        if (data.type == 'formula') { // 进入公式保存处理
            ...
            this.saveTask(data.taskId, data.field);
            if (needChgPeriod) {
...
a2DataGrid.prototype.saveTask = function (taskId, field) { // ajax task
    var Dt = this.getTaskData(taskId), postDt = {}, sf = this;
        ...
        success: function (rs) {
            if (!rs.Success) {
                ...
                if (ck) {
                    ...
                    if (field && field == 'ownerName') { // 修改部门
                        var ds = sf.filterDs, htInfo = rs.htInfo, orgName;
                        if (htInfo && (orgName = htInfo.orgName)) {
                            if (orgName != Dt['orgName']) {
                                sf.updateFieldData(taskId, 'orgName', orgName);
                                sf.updateView(taskId, 'orgName', orgName);
                            }
                            var departs = ds.departs;
                            departs[orgName] = departs[orgName] || [];
                            departs[orgName].push(Dt['ownerName']);
                            sf._renderFilter();
                        }
                    }
                }
        ...
        submit_cb: function (o) {
            ...
            if (confirm("载入模板将删除已有任务及其数据,确定载入吗？")) {
                ...
                $.ajax({
                    ...
                    complete: function () { o.fbox.unmask(); o.destroy(); },
                    success: function (data) {
                        if (data.Success) {
...
function _gFilter(sf, o) {
    var pne = $('#_g_filterPne'), field = o.getAttribute('field'),
        _arr = ['<li><input type="checkbox" name="filtergrps" value="g_gall" />全选</li>'],
        ds = sf.filterDs, key, item, ct;
    switch (field) {
        case 'levelId':
            if (typeof xhLevel != 'undefined') {
                for (key = 0; item = xhLevel[key]; ++key) {
                    if (item.lvlid != '-1') _arr.push('<li><input type="checkbox" name="filtergrps" value="' + item.lvlname + '" />' + item.lvlname + '</li>');
                }
            }
            break;
        case 'status':
            for (key in ds.stat) {
                _arr.push('<li><input type="checkbox" name="filtergrps" value="' + key + '" />' + ds.stat[key] + '</li>');
            }
            break;
    }
    field != 'status' && _arr.push('<li><input type="checkbox" name="filtergrps" value="g_gblank" />空白</li>');
    ct = pne.find('ul:first');
    ct.html(_arr.join(''));

    var w = pne.width(), h = pne.height(), $tag = $(o), offset = $tag.offset(), _h = $tag.height(), _w = $tag.width(), _t = offset.top, _l = offset.left, tbody = sf.dt[0], _idx = $tag.index(), str = '';

    pne.off().on('click', 'input', function () {
        if (this.type.toLowerCase() == 'checkbox') {
            if (this.value == 'g_gall') {
                if ($(this).attr("checked")) {
                    pne.find("input[type='checkbox']").attr("checked", "checked");
                } else {
                    pne.find("input[type='checkbox']").removeAttr("checked");
                }
            } else {
                if (!$(this).attr("checked")) pne.find("input[type='checkbox']:first").removeAttr("checked");
            }
        } else {
            if (this.name == 'cancellvl') {
                pne.hide();
            } else if (this.name == 'confirmlvl') {
                str = "|YYY|";
                pne.find("input[type='checkbox']").each(function () {
                    if ($(this).attr("checked")) str += $(this).val() + "|YYY|";
                });
                _doGFilter(sf, pne, tbody, str, _idx, field);
                sf.focusDiv.hide();
            }
        }
    });
    ...
    pne.css({ left: _l - w + _w, top: _t + _h }).show();
}

function _doGFilter(sf, pne, tbody, str, idx, field) {
    pne.hide();
    if (str == "|YYY|") return;
    var tr, lvl, i = 0, _rows = tbody.rows, tid;

    for (; tr = _rows[i]; ++i) {
        if (field == 'status') {
            lvl = tr.cells[idx].getAttribute('statId');
        } else {
            lvl = tr.cells[idx].innerHTML;
        }
        tid = tr.id.replace('tr', '');
...
function _doGFilterDept(sf, str) {
    if (str == '|YYY|') return;
    var tr, d, u, i = 0, _rows = sf.dt[0].rows, tid;

    for (; tr = _rows[i]; ++i) {
        d = tr.cells[6].innerHTML; // 部门
        if (d == '') continue;
        u = tr.cells[5].innerHTML; // 用户
        tid = tr.id.replace('tr', '');

        if (u != "") {
            u = "|YYY|" + d + '|DDD|' + u + "|YYY|";
            if (str.indexOf(u) == -1) { // 移除
                sf.unSelRow_(tr, tid);
            } else {
                sf.SelRow_(tr, tid);
            }
