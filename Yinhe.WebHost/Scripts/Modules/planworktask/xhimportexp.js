...
var a3plan_impexp = (function() {
    var lck,
		urls = {
		    ...
		    iExpLoad: '/DesignManage/PlanAndTaskList/', // 获取对应公司经验的任务结构
		    iExpSave: '/DesignManage/QuickyCreatePlan' // 导入到计划
		},
        taskStatus = ["待分解", "分解完成", "未开始", "进行中", "已完成"],
		tplv = '<tr><td>{rowindex}</td><td><div style="padding-left:{padleft}px;padding-right:5px;float:left"><img style="{icon}" src="/Content/Images/treeview/icotree/ico-Expand.gif"></div><div style="float:left">{name}</div></td><td width="80">{ownerName}</td><td width="100">{planStartDate}</td><td width="100">{planEndDate}</td><td></td></tr>';
    ...
    function __importExp(force, projID, fromPlan) { // 导入经验
        ...
        var expData = [], _secdPlanId = -1, secdPlanId = 0, setoffset_year, setoffset_month, setoffset_day, isIgnorePerson, isIgnoreDate, isIgnorePassed;
        var _u = fromPlan ? urls.importExpFromPlan : urls.importExp;
        box(_u + secdPlanId + "?projID=" + projID, { boxid: 'loadExp', title: '载入公司经验', contentType: 'ajax', modal: true, submit_BtnName: '确定导入', cls: 'shadow-container7',
            onLoad: function(o) {
                //o.fbox.center();
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                if (data.Success) {
                                    ...
                                    var gourl = '/Projects/ProjArranged/PlanArrange?projId=';
