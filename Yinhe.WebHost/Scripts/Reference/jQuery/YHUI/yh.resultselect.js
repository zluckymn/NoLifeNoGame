(function($) {
    $.Perc = function() {
        ...
        var multiRs = function(id, name, ckd) {
            ...
            li.click(function() {
                ...
                $.PercSetup.updateSubmitBtn(true);
            });
        };
        var render = function(data, nclear, ids, utp, dtp) {
            ...
            if (dty == "normal") _cSearchBox(data.length, ul, pLi);
            if (dty == "tree") _SearchTree(ul, pLi);
            ...
            if (dty == "normal") { // 平级
                ...
                for (var I = 0; I < data.length; I++) {
                    ...
                    var li = $("<li/>").addClass("sli").attr({ "sid": rs.id, "dtype": rs.dtype, "utype": rs.utype });
                    u = rs.utype;
                    ...
                    ul.append(li);
                    (function() { // li click
                        var obj = li, id = rs.id, name = rs.text, lf = leaf, urlt = u, tids = uids;
                        obj.click(function() {
                            ...
                            if ((urlt == opt.endFlag || lf == 1) && opt.multiSel) {
                                ...
                                if (opt.showRsBox) multiRs(id, name, ckd);
                            } else {
        ...
        return {
            init: function() {
                ...
                if (opt.showRsBox) {
                    var arr = opt.initRSData || [];
                    for (var i = 0; i < arr.length; i++) {
                        multiRs(arr[i].id, arr[i].name, true);
                    }
                }
    ...
    $.PercSetup = function() {
        var opt = {};
        var cbox = function() {
            var w = (opt.valListWidth + opt.valAdjustWidth) * opt.iCds - opt.valAdjustWidth - 2, wb = w;
            var tpl = [
            ...
            var bx = box(tpl, { boxid: "perc", title: opt.title, width: wb + 42,
                submit_cb: function(o) {
                    ...
                    if (rsc.find("div.g_ktree")[0]) { // tree
                        ...
                        rsc.find("input:checked").each(function() {
                            var v = $(this).val(), txt = $(this).parent().text();
                            response[v] = txt;
                        });
                    }
