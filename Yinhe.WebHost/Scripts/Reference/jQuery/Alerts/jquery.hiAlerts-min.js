(function(a) {
    a.alerts = {
        ...
        overAlert: function(c, b) { a.alerts._overShow(c, b) },
        ...
        _overShow: function(d, c) {
            ...
            if (a.alerts.dialogClass) { a("#over_container").addClass(a.alerts.dialogClass) }
            ...
            a.alerts._overReposition(); setTimeout(function() { if (!document.getElementById("hiOverAlertClose")) { a("#over_container").hide("fast") } }, c);
            setTimeout(function() { if (!document.getElementById("hiOverAlertClose")) { a("#over_container").remove() } }, b)
        },

        _overReposition: function() {
            var c = 4; var b = ((a(window).width() / 2) - (a("#over_container").outerWidth() / 2)) + a.alerts.horizontalOffset;
            if (c < 0) { c = 0 }
            if (b < 0) { b = 0 }
        ...
        _show: function(j, b, k, g, m, l, c, f, n) {
            ...
            if ((a.browser.msie && parseInt(a.browser.version) <= 6)) {
                if (document.getElementById('hiAlertHiddenSelect')) {

                }
                else {
                    $("body").append('<iframe id="hiAlertHiddenSelect" scrolling="no"   frameborder="0"  style="z-index:99998;position:absolute; top:0px; right:0px; display:none; background-color:Black;"></iframe> ');
                }
            }
            ...
            if (g != "openBox") { a("#popup_message").text(b).html(a("#popup_message").text().replace(/\n/g, "<br />")) }
            ...
            switch (g) {
                case "alert":
                    ...
                    a("#popup_ok").focus().keypress(function(h) { if (h.keyCode == 13 || h.keyCode == 27) { a("#popup_ok").trigger("click") } });
                    break;
                ...
                case "confirm":
                    ...
                    a("#popup_ok").click(function() { a.alerts._hide(); if (m) { m(true) } });
                    a("#popup_cancel").click(function() { a.alerts._hide(); if (m) { m(false) } });
                    a("#popup_ok").focus();
                    a("#popup_ok, #popup_cancel").keypress(function(h) { if (h.keyCode == 13) { a("#popup_ok").trigger("click") } if (h.keyCode == 27) { a("#popup_cancel").trigger("click") } });
                ...
                case "prompt":
                    ...
                    if (k) { a("#popup_prompt").val(k) } a("#popup_prompt").focus().select(); break; case "openBox": a("#popup_message").append(a(b).html()); if (l) { a("#popup_container").css({ width: l + "px" }) } if (c) { a("#popup_container").css({ height: c + "px" }); a("#popup_message").css({ height: (c - 48) + "px" }) } a.alerts._reposition(); if (f) { a(f).click(function() { a.alerts._hide(); if (m) { m(true) } }) } if (n) { a(n).click(function() { a.alerts._hide(); return false; if (m) { m(false) } }) } break
            }
            a("#popup_close").click(function() { a.alerts._hide(); if (m) { m(false) } });
            var temp = a.alerts;
            temp._hiddenSelect();
            if (a.alerts.draggable) {
                try {
                    a("#popup_container").draggable({ handle: a("#popup_title"), drag: function(event, ui) {
                        temp._hiddenSelect();
                    }
                    });
            ...
            a("#hiAlertHiddenSelect").hide();
            a.alerts._overlay("hide");
            a.alerts._maintainPosition(false)
        },
        _setOpacity: function(obj, alpha) {
            var style = a(obj)[0].style;
            if ('opacity' in style) {
                style.opacity = alpha;
        ...
        _overlay: function(b) {
            switch (b) {
                case "show": a.alerts._overlay("hide");
                    ...
                    a("#popup_overlay").css({ position: "absolute", zIndex: 99998, top: "0px", left: "0px", width: "100%", height: a(document).height(), background: a.alerts.overlayColor });
                    a.alerts._setOpacity(a("#popup_overlay")[0], a.alerts.overlayOpacity);
                    break;
                case "hide":
        ...
        _hiddenSelect: function() {

            if (a.browser.msie && parseInt(a.browser.version) <= 6) {

                var offset = a("#popup_container").offset();
                var c = offset.top;
                var b = offset.left;
                a("#hiAlertHiddenSelect").css({ top: (c) + "px", left: (b) + "px", display: "block" });
                a("#hiAlertHiddenSelect").height(parseInt(a("#popup_container").height()) + 2);
                a("#hiAlertHiddenSelect").width(parseInt(a("#popup_container").width()) + 2)
                a("#popup_overlay").hide();
            }
    ...
    hiAlert = function(b, c, d) { $("#working").hide(); a.alerts.alert(b, c, d) };
    hiConfirm = function(b, c, d) { $("#working").hide(); a.alerts.confirm(b, c, d) };
    hiPrompt = function(b, c, d, e) { $("#working").hide(); a.alerts.prompt(b, c, d, e) };
    hiBox = function(f, g, b, c, e, d, i) { $("#working").hide(); a.alerts.openBox(f, g, b, c, e, d, i) };
    hiOverAlert = function(c, b) { $("#working").hide(); a.alerts.overAlert(c, b) }
})(jQuery);
