myFocus.extend({//*********************fscreen******************
    mF_fscreen_tb: function (par, F) {
        ...
        //CSS
        var pic = F.$li('pic', box), winW = par.width - 20, sw = 96, sh = 56; //参考*按比例求高
        ...
        $(".thumb img").each(function () {
            if ($(this).attr("src").indexOf("_st") != -1) {
                $(this).attr("src", $(this).attr("src").replace("_st", "_hs"));
            }
        });
        ...
        eval(F.switchMF(function () {
            ...
            if (pic.length > 6) eval(F.scroll('thus', '"left"', 'sw', 4));
            if (window.SetPicInfo) SetPicInfo(next);
        }));
